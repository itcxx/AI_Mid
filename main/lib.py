...
def preprocess_nmt(text):
    """Preprocess the Korean  english  dataset.#预处理数据集

    Defined in :numref:`sec_machine_translation`"""
    def no_space(char, prev_char):
        return char in set(',.!?') and prev_char != ' '   #如果是标点符号，且前一个字符不是空格，返回True
...
def tokenize_nmt(text, num_examples=None):   #分词，词源化
    """Tokenize the korean english dataset.
...
def truncate_pad(line, num_steps, padding_token):
    ...
    if len(line) > num_steps:
        return line[:num_steps]  # Truncate #截断大于num_steps的部分
    return line + [padding_token] * (num_steps - len(line))  # Padding_token #填充小于num_steps的部分

def build_array_nmt(lines, vocab, num_steps): #将句子换成小批量
    """Transform text sequences of machine translation into minibatches.

    Defined in :numref:`subsec_mt_data_loading`"""
    lines = [vocab[l] for l in lines]
    lines = [l + [vocab['<eos>']] for l in lines]  #将句尾加入截至符
